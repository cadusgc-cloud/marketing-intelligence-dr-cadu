...
import { AppShell, PageHeader, ProgressBar } from "@/components/product";
...
export function FlowRunnerClient({ flow }: { flow: GuidedFlow }) {
  ...
  return (
    <AppShell>
      <PageHeader eyebrow="Fluxo guiado" title={flow.name} description={flow.description}>
        <p className="mt-2 text-xs text-slate-500">{storageStatus} | {flow.estimatedMinutes} min | {flow.complexity}</p>
        <div className="mt-4 flex flex-wrap gap-2">
          <button type="button" onClick={start} className="rounded-md bg-ocean px-4 py-2 text-sm font-semibold text-white hover:bg-cyan-800">Iniciar fluxo</button>
          <button type="button" onClick={reset} className="rounded-md border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Resetar progresso</button>
      ...
      <section className="panel">
        <ProgressBar value={run.progressPercent} label="Progresso do fluxo" />
      </section>
        ...
        <section className="panel">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold">Resumo do fluxo</h2>
            </div>
        ...
        <section className="panel">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-lg font-semibold">Conclusao local</h2>
            </div>
