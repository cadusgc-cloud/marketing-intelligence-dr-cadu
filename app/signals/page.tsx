  ...
  return (
    <div className="space-y-6">
      <section className="panel">
        <p className="text-sm font-medium text-ocean">Sinais de decisão</p>
        <h2 className="mt-1 text-2xl font-semibold">Sinais de decisão</h2>
        <p className="mt-2 text-sm text-slate-500">Regras operacionais para transformar dados de marketing em decisões práticas.</p>
      </section>

      <section className="grid gap-3 md:grid-cols-6">
        <MetricCard label="Sinais acionados" value={triggered.length} />
        <MetricCard label="Sinais críticos" value={critical.length} />
        <MetricCard label="Decisões de escala" value={byDecisionType.scale} />
        <MetricCard label="Pausa/redução" value={byDecisionType.pause + byDecisionType.reduce} />
        <MetricCard label="Precisa de dados" value={needsDataSignals.length} />
        <MetricCard label="Regras avaliadas" value={results.length} />
      </section>

      <section className="grid gap-6 lg:grid-cols-3">
        <div className="panel lg:col-span-2">
          <h3 className="text-lg font-semibold">Diagnóstico executivo dos sinais de decisão</h3>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-md bg-slate-50 p-3">
              ...
              <p className="mt-1 text-sm text-slate-600">Canal principal de escala quando o BOFU entrega conversas no WhatsApp com bom custo.</p>
            </div>
            <div className="rounded-md bg-slate-50 p-3">
              <p className="text-sm font-semibold">Google Ads</p>
              <p className="mt-1 text-sm text-slate-600">Permanece em diagnóstico até as conversões ficarem confiáveis.</p>
            </div>
            <div className="rounded-md bg-slate-50 p-3">
              <p className="text-sm font-semibold">Conteúdo</p>
              <p className="mt-1 text-sm text-slate-600">Stories, Reels/Shorts e TikTok sustentam o funil e a rotina.</p>
            </div>
          </div>
        </div>
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="panel">
          <h3 className="text-lg font-semibold">Decisões recomendadas agora</h3>
        ...
        <div className="panel">
          <h3 className="text-lg font-semibold">Não escalar ainda</h3>
          <ul className="mt-3 space-y-2 text-sm text-slate-600">
                ...
                {channelSignals.map((signal) => (
                  <article key={signal.id} className="rounded-lg border border-slate-200 p-4">
                    <div className="flex flex-wrap gap-2">
                      ...
                      <span className="badge bg-slate-100 text-slate-700">Confiança {signal.confidence}</span>
                    </div>
                    ...
                    <div className="mt-3 rounded-md bg-slate-50 p-3 text-sm text-slate-600">
                      <p><span className="font-semibold">Próxima ação:</span> {signal.nextAction}</p>
      ...
      <section className="panel">
        <h3 className="text-lg font-semibold">Filtros de domínio disponíveis</h3>
        <p className="mt-2 text-sm text-slate-600">
          O módulo possui filtros testáveis por canal, severidade e tipo de decisão. Exemplo: {filterSignalsBySeverity(triggered, "critical").length} sinal(is) crítico(s) e{" "}
          {filterSignalsByDecisionType(triggered, "investigate").length} sinal(is) de investigação.
        </p>
      </section>
    </div>
  );
