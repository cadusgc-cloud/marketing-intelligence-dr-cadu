...
describe("generateExecutiveDiagnosis", () => {
  ...
  it("desativa score e recomendacoes executivas para relatorio anomalo", () => {
    const diagnosis = generateExecutiveDiagnosis({
      report: {
        title: "Dezembro 2025",
      ...
      creatives: [
        { name: "Criativo vencedor sintetico", diagnosis: "scale", cpl: 4.5, leads: 12 },
        { name: "Criativo problematico sintetico", diagnosis: "pause", investment: 250, leads: 1 }
      ],
      recommendations: [
        recommendation({
          category: "google_ads",
          priority: "critical",
          title: "Google Ads critico",
      ...
      dataIssues: [
        dataIssue({
          ...
          description: "Periodo mantido como contexto historico"
        })
      ]
    });
    ...
    expect(diagnosis.criticalAlerts).toEqual([]);
    expect(diagnosis.topWins).toEqual([]);
    expect(diagnosis.scalePoints).toEqual([]);
    expect(diagnosis.wastePoints).toEqual([]);
    expect(diagnosis.nextWeekActionPlan).toEqual([]);
